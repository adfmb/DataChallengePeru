...
```{r}
clases<-as.data.frame(base00)%>%
  mutate(fh_cotizacion=as.character(fh_cotizacion))%>%
  summarise_all(class)#apply(head(base00),1,class)
df_clases<-data_frame(
  "campos"=names(base00),
  "clase"=clases
)
df_clases
...
saveRDS(base00_train,"base00_train.rds")
```

```{r}
base00_test<-base00%>%
          filter(muestra=="test")
saveRDS(base00_test,"base00_test.rds")
```
